 ...
 *
 * A PumpkinSection publishes its layout when it mounts and drops it when it
 ...
 * render, so PumpkinField must stay ahead of CollectibleField in the scene
...
export const publishSectionPumpkins = (
    sectionIndex: number,
...
export const getSectionPumpkins = (sectionIndex: number): THREE.Vector3[] =>
    sections.get(sectionIndex) ?? [];

export const releaseSectionPumpkins = (sectionIndex: number) => {
